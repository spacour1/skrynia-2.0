...
export default function HomePage() {
  ...
  const trustItems = [
    { title: t("home.trustEscrowTitle"), text: t("home.trustEscrowText"), icon: ShieldCheck },
    { title: t("home.trustSupportTitle"), text: t("home.trustSupportText"), icon: Headphones },
    { title: t("home.trustReviewsTitle"), text: t("home.trustReviewsText"), icon: Star },
    { title: t("home.trustInstantTitle"), text: t("home.trustInstantText"), icon: Zap }
  ];

  const serviceCards = [
    [t("home.serviceLeveling"), t("home.serviceLevelingText"), "1240+"],
    [t("home.serviceBoosting"), t("home.serviceBoostingText"), "980+"],
    [t("home.serviceQuests"), t("home.serviceQuestsText"), "870+"],
    [t("home.serviceDelivery"), t("home.serviceDeliveryText"), "760+"]
  ];
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <main className="relative z-0 space-y-6">
        ...
        <section id="game-catalog" className="space-y-4 scroll-mt-28">
          <div className="app-card overflow-hidden">
            <div className="flex flex-col gap-4 border-b border-line bg-panel/55 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase text-brand">{t("home.gamesAndServices")}</p>
                <h2 className="mt-1 text-2xl font-black text-ink">{t("home.catalogTitle")}</h2>
                <p className="mt-1 text-sm text-muted">{t("home.catalogText")}</p>
              </div>
              <button className="app-button-secondary h-11 px-4" onClick={() => setShowAllGames((value) => !value)}>
                {showAllGames ? t("home.collapse") : t("home.showAllCount", { count: gamesList.length })}
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-3 flex items-center justify-between text-xs font-bold text-muted">
                      <span>{selected ? t("home.selected") : t("home.openSections")}</span>
            ...
            <section className="app-card overflow-hidden">
              <div className="flex flex-col gap-4 border-b border-line bg-panel/45 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-ink">{t("home.sectionsOf", { game: activeGameName ?? "" })}</h3>
                    <p className="text-sm text-muted">{t("home.sectionsText")}</p>
                  </div>
                </div>
                <button className="app-button-secondary h-10 px-4" onClick={() => setSection("")}>{t("home.allSections")}</button>
              </div>
        ...
        <section className="app-card p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-xl font-extrabold">{t("home.popularServices")}</h2>
            <button className="inline-flex items-center gap-1 text-sm font-bold text-brand" onClick={() => { setSort("sales"); scrollToProducts(); }}>
              {t("home.viewAllArrow")} <ChevronRight className="h-4 w-4" />
            </button>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-4">
            {serviceCards.map(([title, text, count]) => (
        ...
        <section id="popular-products" className="space-y-4 scroll-mt-28">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-extrabold">{showFavorites ? t("home.favoriteProducts") : t("home.popularProducts")}</h2>
              <p className="mt-1 text-sm text-muted">
                {showFavorites ? t("home.likedProductsText") : activeGameName ? t("home.filterPrefix", { game: activeGameName }) : t("home.freshOffers")}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <select className="app-input py-2" value={sort} onChange={(event) => setSort(event.target.value)} disabled={showFavorites}>
                ...
                <option value="sales">{t("home.sortSales")}</option>
                <option value="discount">{t("home.sortDiscount")}</option>
                <option value="price_asc">{t("home.sortPriceLow")}</option>
                <option value="price_desc">{t("home.sortPriceHigh")}</option>
          ...
          {(products.isLoading || favorites.isLoading) ? (
            <section className="app-card p-8 text-center text-muted">{t("home.loadingProducts")}</section>
          ) : productsList.length ? (
            ...
            <section className="app-card p-8 text-center text-muted">
              {showFavorites ? t("home.noFavorites") : t("home.noProducts")}
            </section>
          )}
        </section>

        <section className="app-card grid gap-4 bg-panel/70 p-5 md:grid-cols-3">
          <FeatureLine icon={ShieldCheck} title={t("home.featureSecureTitle")} text={t("home.featureSecureText")} />
          <FeatureLine icon={BadgeCheck} title={t("home.featureRatingTitle")} text={t("home.featureRatingText")} />
          <FeatureLine icon={Headphones} title={t("home.featureSupportTitle")} text={t("home.featureSupportText")} />
        </section>
      </main>
      ...
      <aside className="space-y-5 xl:sticky xl:top-[106px] xl:self-start">
        <section className="app-card overflow-hidden">
          <div className="p-5">
            <h2 className="text-xl font-extrabold">{t("home.recentActivity")}</h2>
            <div className="mt-5 space-y-5">
              {user ? (
                orders.data?.orders?.length ? (
                  ...
                  <p className="text-sm leading-6 text-muted">{t("home.noActivity")}</p>
                )
              ) : (
                <p className="text-sm leading-6 text-muted">{t("home.loginToSeeActivity")}</p>
              )}
            </div>
          </div>
        ...
        >
          ...
          <span className="mt-4 block text-lg font-black text-ink">{t("home.generalChat")}</span>
          <span className="mt-2 block text-sm leading-6 text-muted">{t("home.generalChatText")}</span>
        </button>
      </aside>
    </div>
  );
    ...
    >
      ...
      <div className="relative p-4">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <p className="text-xs text-muted">{t("home.soldCount", { count: product.salesCount ?? 0 })}</p>
        </div>
        <p className="mt-2 text-xs text-muted">{t("home.likesCount", { count: product.favoriteCount ?? 0 })}</p>
      </div>
    </article>
  );
}

function pluralLots(count: number, t: (key: string) => string) {
  const mod10 = count % 10;
  const mod100 = count % 100;
  if (mod10 === 1 && mod100 !== 11) return t("home.lotsOne");
  if (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14)) return t("home.lotsFew");
  return t("home.lotsMany");
}

function CatalogMegaMenu({ games, onGame }: { games: Game[]; onGame: (slug: string) => void }) {
  const { t } = useI18n();
  const groups = [
    { title: t("home.menuGames"), text: t("home.menuGamesText"), items: games.slice(0, 8), live: true },
    { title: t("home.menuMobile"), text: t("home.menuMobileText"), items: games.filter((game) => /mobile|genshin|roblox/i.test(game.name)).slice(0, 5), live: true },
    { title: t("home.menuServices"), text: t("home.menuServicesText"), items: [] as Game[], live: false },
    { title: t("home.menuSoftware"), text: t("home.menuSoftwareText"), items: [] as Game[], live: false }
  ];
