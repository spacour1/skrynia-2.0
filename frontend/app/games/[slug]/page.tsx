...
export default function GameCatalogPage({ params }: { params: { slug: string } }) {
  ...
  const gameDetail = useQuery({
    ...
    queryFn: () => apiFetch<{ game: Game; sections: GameSection[] }>(`/marketplace/games/${params.slug}`)
  });

  const query = useMemo(() => {
    const search = new URLSearchParams();
    search.set("game", params.slug);
  ...
  }, [params.slug, section, productType, autoDeliveryOnly, sort]);
  ...
  const sectionRows = buildSectionRows(sections, total);
    ...
    <div className="mx-auto max-w-[1380px] space-y-5 text-slate-100">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          ...
          <GameIcon name={game?.name ?? params.slug} slug={params.slug} className="h-16 w-16 rounded-xl" />
          <div className="min-w-0">
            <h1 className="truncate text-2xl font-black text-slate-100 md:text-3xl">{game?.name ?? "Загрузка..."}</h1>
            <p className="mt-1 text-sm font-semibold text-slate-400">Всего объявлений: {total.toLocaleString("ru-RU")}</p>
          </div>
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="flex items-center gap-3">
            <p className="whitespace-nowrap text-sm font-bold text-slate-500">Сортировать за:</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <section className="overflow-hidden rounded-lg border border-slate-800 bg-slate-950/80 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-black text-slate-100">Фильтры</h2>
              </div>
              <button
                ...
                <div className="space-y-1">
                  {sectionRows.map((item) => (
                    ...
                    >
                      ...
                      <span className="font-black text-slate-500">{item.count.toLocaleString("ru-RU")}</span>
                    </button>
                  ))}
                </div>
              </FilterBlock>

              <FilterBlock label="Автовыдача">
                <label className="flex cursor-pointer items-center gap-3 rounded-md px-1 py-2 text-sm font-semibold text-slate-400 transition hover:text-slate-100">
                  ...
                  />
                  Автовыдача после покупки
                </label>
              </FilterBlock>

              <FilterBlock label="Продавцы">
                <label className="flex cursor-pointer items-center gap-3 rounded-md px-1 py-2 text-sm font-semibold text-slate-400 transition hover:text-slate-100">
                  <input
                    className="h-4 w-4 rounded border-slate-700 bg-slate-950 accent-amber-400"
                    type="checkbox"
        ...
        <main className="min-w-0">
          <section className="overflow-hidden rounded-lg border border-slate-800 bg-slate-950/65">
            {products.isLoading ? <div className="p-10 text-center text-sm font-bold text-slate-500">Загружаем предложения...</div> : null}
            {!products.isLoading && !visibleList.length ? (
              <div className="grid min-h-[260px] place-items-center p-10 text-center">
                <div>
                  ...
                  <h2 className="mt-4 text-xl font-black text-slate-100">Предложения не найдены</h2>
                  <p className="mt-2 text-sm text-slate-500">Попробуйте сбросить фильтры или выбрать другой раздел.</p>
                </div>
              </div>
            ) : null}
...
function OfferRow({
  ...
  const tenure = index % 3 === 0 ? "На сайте 1 год" : index % 3 === 1 ? "На сайте 2 года" : "На сайте 8 мес.";
  const metrics = offerMetrics(product, index);
      ...
      <div className="relative grid gap-4 px-4 py-4 md:grid-cols-[minmax(0,1fr)_190px_120px_44px] md:items-center lg:px-6">
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-black">
            {product.deliveryType === "instant" ? <span className="rounded bg-emerald-500/10 px-2 py-1 text-emerald-400">Автовыдача</span> : null}
            {discount ? (
...
function buildSectionRows(sections: GameSection[], total: number) {
  ...
  if (normalizedSections.length) {
    return [{ value: "", label: "Все разделы", count: total, kind: "all" as const }, ...normalizedSections];
  }
...
function offerMetrics(product: Product, index: number) {
  const base = Number(product.priceCents ?? 0);
  return [
    { icon: "🔑", label: `${Math.max(1200, Math.round(base / 12))} MMR` },
    { icon: "🏆", label: `${Math.max(2500, Number(product.salesCount ?? 0) * 150 + 5000)} порядочности` },
    { icon: index % 2 ? "🍀" : "🛡️", label: `${Math.max(3000, Math.round(base / 8))} вежливости` }
  ];
}
