...
function FavoritesContent() {
  ...
  const removeFavorite = useMutation({
    ...
    onSuccess: (_data, productId) => {
      ...
      showAppToast({
        type: "favorite",
        title: "Объявление удалено из Избранных",
        productId
      });
    }
  });
  ...
  return (
    <div className="mx-auto max-w-[1120px] space-y-5">
      <header className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-black text-ink">Избранные</h1>
      </header>

      {favorites.isLoading ? <section className="app-card p-8 text-center text-muted">Загружаем избранные объявления...</section> : null}
      ...
      {!favorites.isLoading && !products.length ? (
        <section className="app-card grid min-h-[300px] place-items-center p-8 text-center">
          <div>
            ...
            <h2 className="mt-4 text-xl font-black text-ink">Пока нет избранных объявлений</h2>
            <p className="mt-2 text-sm text-muted">Нажмите на сердечко у объявления, и оно появится здесь.</p>
          </div>
        </section>
      ) : null}
    </div>
