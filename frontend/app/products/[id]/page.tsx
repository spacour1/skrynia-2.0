...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const product = useQuery({
    ...
    queryFn: () => apiFetch<{ product: Product; reviews: ProductReview[] }>(`/marketplace/products/${params.id}`)
  });

  const productItem = product.data?.product;
  const autoChat = useQuery({
    queryKey: ["product-chat", params.id, user?.id],
    queryFn: () => apiFetch<{ conversationId: string; existing: boolean }>(`/chat/products/${params.id}/start`, { method: "POST" }),
