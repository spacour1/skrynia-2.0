...
const tabs = [
  ["all", "common.all"],
  ["wallet_credit", "wallet.tabTopups"],
  ["payment_capture", "wallet.tabPurchases"],
  ["escrow_release", "wallet.tabSales"],
  ["wallet_debit", "wallet.tabWithdrawals"],
  ["escrow", "Escrow"]
];
  ...
  return (
    <div className="mx-auto max-w-[1180px] space-y-8">
      <section className="space-y-2">
        <h1 className="text-3xl font-black text-ink">{t("wallet.title")}</h1>
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        <article className="rounded-lg border border-brand/60 bg-card p-6 shadow-[0_0_0_1px_rgba(250,204,21,0.08),0_20px_60px_rgba(0,0,0,0.28)]">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-muted">{t("wallet.availableToWithdraw")}</p>
              <p className="mt-2 text-3xl font-black text-ink">{money(primary.availableCents, primary.currency)}</p>
              <p className="mt-2 text-sm text-muted">{t("wallet.withdrawNow")}</p>
            </div>
          </div>
          <div className="mt-6 grid gap-3">
            ...
            {topupOpen ? (
              <div className="rounded-lg border border-line bg-panel/35 p-3">
                <label className="block space-y-2">
                  <span className="block text-xs font-bold text-muted">{t("wallet.topupAmountLabel", { currency: primary.currency })}</span>
                  <input
                ...
                <div className="mt-3 grid gap-2">
                  <button className="app-button-action h-11 w-full" disabled={!topupAmount || topupWithLiqpay.isPending} onClick={() => topupWithLiqpay.mutate()}>
                    {topupWithLiqpay.isPending ? t("wallet.payingRedirect") : t("wallet.payWith", { provider: "LiqPay" })}
                  </button>
                  <button className="app-button-action h-11 w-full" disabled={!topupAmount || topupWithMonobank.isPending} onClick={() => topupWithMonobank.mutate()}>
                    {topupWithMonobank.isPending ? t("wallet.payingRedirect") : t("wallet.payWith", { provider: "Monobank" })}
                  </button>
                  <button className="app-button-action h-11 w-full" disabled={!topupAmount || topupWithWayforpay.isPending} onClick={() => topupWithWayforpay.mutate()}>
                    {topupWithWayforpay.isPending ? t("wallet.payingRedirect") : t("wallet.payWith", { provider: "WayForPay" })}
                  </button>
                </div>
                {topupError ? (
            ...
            {withdrawOpen ? (
              <div className="rounded-lg border border-line bg-panel/35 p-3">
                <label className="block space-y-2">
                  <span className="block text-xs font-bold text-muted">{t("wallet.withdrawAmountLabel", { currency: primary.currency })}</span>
                  <input
                    className="app-input h-11 w-full"
                    type="number"
                    min="1"
                ...
                <label className="mt-3 block space-y-2">
                  <span className="block text-xs font-bold text-muted">{t("wallet.cardNumberLabel")}</span>
                  <input
                    className="app-input h-11 w-full"
                    type="text"
                ...
                <label className="mt-3 block space-y-2">
                  <span className="block text-xs font-bold text-muted">{t("wallet.holderLabel")}</span>
                  <input
                    className="app-input h-11 w-full"
                    type="text"
                ...
                <p className="mt-2 text-xs text-muted">{t("wallet.withdrawNote")}</p>
                <button
                ...
                >
                  {withdraw.isPending ? t("wallet.sendingRequest") : t("wallet.confirmWithdraw")}
                </button>
                {withdraw.error ? (
        ...
        <BalanceCard icon={ShieldCheck} label={t("wallet.inEscrow")} value={money(primary.escrowCents, primary.currency)} text={t("wallet.escrowPendingText")} accent="text-brand" />
        <BalanceCard icon={Hourglass} label={t("wallet.processing")} value={money(processingCents, primary.currency)} text={t("wallet.processingText")} accent="text-sky-400" />
      </section>
      ...
      <section className="app-card overflow-hidden">
        <div className="flex flex-col gap-4 border-b border-line bg-panel/45 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-black text-ink">{t("wallet.history")}</h2>
            <div className="relative mt-4 w-full lg:w-[360px]">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" />
              <input className="app-input h-11 w-full pl-10" value={q} onChange={(event) => setQ(event.target.value)} placeholder={t("wallet.searchPlaceholder")} />
            </div>
          </div>
        ...
        <div className="overflow-x-auto p-5">
          <table className="min-w-[820px] w-full border-separate border-spacing-0">
            <thead>
              <tr className="text-left text-xs font-bold uppercase text-muted">
                <th className="py-3">{t("wallet.operation")}</th>
                <th className="py-3">{t("common.date")}</th>
                <th className="py-3">{t("common.status")}</th>
                <th className="py-3 text-right">{t("common.amount")}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((tx) => (
                <tr key={tx.id} className="border-b border-line">
                  <td className="border-t border-line py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span>
                        <span className="block font-bold text-ink">{labelTx(tx.type, t)}</span>
                        <span className="mt-1 block text-xs text-muted">
                          {tx.orderId ? t("wallet.orderRef", { id: tx.orderId.slice(0, 8) }) : tx.direction === "credit" ? t("wallet.credit") : t("wallet.debit")}
                        </span>
                      </span>
                    </div>
                  </td>
                  <td className="border-t border-line py-4 text-sm text-muted">{formatDate(tx.createdAt, language)}</td>
                  <td className="border-t border-line py-4">
                    <span className={`rounded px-2 py-1 text-xs font-bold ${statusClass(tx.status, tx.type)}`}>{statusLabel(tx.status, tx.type, t)}</span>
                  </td>
          ...
          {!wallet.isLoading && !filtered.length ? <p className="py-10 text-center text-sm text-muted">{t("wallet.historyEmpty")}</p> : null}
          {wallet.isLoading ? <p className="py-10 text-center text-sm text-muted">{t("wallet.loadingTx")}</p> : null}
        </div>
      </section>

      <p className="flex items-center justify-center gap-2 text-sm text-muted">
        <LockKeyhole className="h-4 w-4" />
        {t("wallet.securePayments")}
      </p>
    </div>
  );
}
...
function labelTx(type: string, t: (key: string) => string) {
  const labels: Record<string, string> = {
    payment_capture: "wallet.txPaymentCapture",
    escrow_hold: "wallet.txEscrowHold",
    escrow_release: "wallet.txEscrowRelease",
    platform_fee: "wallet.txPlatformFee",
    refund: "wallet.txRefund",
    wallet_credit: "wallet.txWalletCredit",
    wallet_debit: "wallet.txWalletDebit"
  };
  const key = labels[type];
  return key ? t(key) : type.replaceAll("_", " ");
}

function statusLabel(status: string, type: string, t: (key: string) => string) {
  if (type.includes("escrow")) return t("wallet.inEscrow");
  if (status === "completed" || status === "succeeded") return t("wallet.statusCompleted");
  if (status === "pending") return t("wallet.processing");
  return status;
}
